...
public class Controller {
	...
	public void listAllProjectsByOwner() {
		get("/list/:email", (request, response) -> {
			
			response.header("Access-Control-Allow-Origin", "*");
	        
	        List<Project> projectsFound = model.searchProjectsByEmailOwner(request.params(":email"));
			
	        return new Gson().toJson(projectsFound);
